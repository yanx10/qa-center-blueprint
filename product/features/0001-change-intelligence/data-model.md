...
| `status` | `text` | No | `'draft'` | `draft`, `ready`, `processing`, `completed`, `failed`, `cancelled` |
| `ai_model` | `text` | Yes | `NULL` | Model identifier used for this analysis (from resolved model chain) |
| `analysis_version` | `text` | Yes | `NULL` | Prompt/workflow version string, e.g., `"m3-v1"` |
| `title` | `text` | No | — | Analysis title; auto-generated if not provided by user (D-023). NOT NULL; no DB default |
| `trigger_type` | `text` | Yes | `NULL` | `'manual'` in M3; reserved for future automation triggers |
| `change_summary` | `text` | Yes | `NULL` | Denormalized from `analysis_json.executive_summary.headline` after successful completion (D-031) |
| `requirement_summary` | `text` | Yes | `NULL` | Denormalized from `analysis_json.executive_summary.primary_concern` after successful completion (D-031) |
| `started_at` | `timestamptz` | Yes | `NULL` | Set when status transitions to `processing` |
| `completed_at` | `timestamptz` | Yes | `NULL` | Set when processing ends (success or failure) |
| `error_code` | `text` | Yes | `NULL` | Error code if analysis failed (e.g., `'PROVIDER_TIMEOUT'`). See error code table in M3 spec. |
| `error_message` | `text` | Yes | `NULL` | Human-readable error message; sanitized — no stack traces or SQL |
| `created_at` | `timestamptz` | No | `NOW()` | |
| `updated_at` | `timestamptz` | No | `NOW()` | Managed by `set_updated_at()` trigger |

**M3 Additions (migration `033_change_intelligence.sql`):**

The following columns are added to `change_analyses` in Milestone 3. All are new additive columns — no existing column is modified.

| Column | Type | Nullable | Default | Description |
|--------|------|----------|---------|-------------|
| `provider` | `text` | Yes | `NULL` | AI provider identifier. Always `'anthropic'` in M3. Reserved for future multi-provider support. |
| `temperature` | `real` | Yes | `NULL` | Temperature parameter used. Fixed at `0.2` in M3 (D-034). |
| `analysis_json` | `jsonb` | Yes | `NULL` | Complete structured AI output. Null until the analysis completes successfully. Never returned in list queries. |
| `analysis_schema_version` | `text` | Yes | `NULL` | Denormalized from `analysis_json.schema_version` on completion. Enables list-level filtering by output schema without JSONB extraction. Example: `"1.0.0"`. |
| `risk_level` | `text` | Yes | `NULL` | Denormalized from `analysis_json.executive_summary.risk_level` on completion. Enables risk-level badge on the list view without loading `analysis_json`. One of: `low`, `medium`, `high`, `critical`. |
| `change_type_summary` | `text` | Yes | `NULL` | Denormalized from `analysis_json.change_summary.change_type` on completion, joined as a comma-separated string (e.g., `"feature, security"`). Enables list-view change-type display without loading `analysis_json`. |
| `input_tokens` | `integer` | Yes | `NULL` | Input tokens consumed from the AI API response. |
| `output_tokens` | `integer` | Yes | `NULL` | Output tokens generated by the AI. |
| `processing_ms` | `integer` | Yes | `NULL` | Total AI call duration in milliseconds. |
| `retry_count` | `smallint` | No | `0` | Number of retry attempts made. Incremented by `POST /retry` (D-032). Maximum value: 3. |

**Notes:**
- The `status` column uses the M2-authoritative values: `draft`, `ready`, `processing`, `completed`, `failed`, `cancelled`. The older values `submitted`, `complete`, and `error` are superseded.
- `change_summary` and `requirement_summary` are denormalized projections from `analysis_json` (D-031). They are populated once on completion and not recomputed on read.
- `analysis_json` is returned by `GET /analyses/:id` only when `status = 'completed'`. It is never included in list responses.
- The `feature_enabled_at_creation` column from earlier blueprint drafts is omitted — it is not needed because the feature flag is checked at request time.
...
| `input_type` | `text` | No | — | `pr_diff`, `pr_description`, `requirement_text`, `jira_story`, `acceptance_criteria`, `prd_text`, `markdown_spec`, `supplemental_context` |
...
| `source_label` | `text` | Yes | `NULL` | Human-readable label for this input (e.g., `'Jira Story QA-1234'`, `'PRD v2.1'`) |
...
## [FUTURE] M4+ Tables — NOT included in M3 migration

The following tables are planned for M4+ milestones. They are **NOT** created by migration `033_change_intelligence.sql`. In M3, the `analysis_json` JSONB field on `change_analyses` stores all AI output. These tables will be introduced when individual finding review is needed (D-028, D-030).
...
| AI-generated QA Intelligence Report | PostgreSQL `jsonb` column (`analysis_json`) | JSONB provides schema flexibility as the prompt evolves; avoids a migration for every prompt change (D-028) |

---

## Migration History

| Migration File | Milestone | Contents |
|---------------|-----------|----------|
| `031_change_intelligence.sql` | M2 — Analysis Persistence Foundation | Creates `change_analyses` and `change_analysis_inputs` tables with all M2 columns, indexes, and constraints |
| `033_change_intelligence.sql` | M3 — AI Change Analysis | Adds 10 columns to `change_analyses`: `provider`, `temperature`, `analysis_json`, `analysis_schema_version`, `risk_level`, `change_type_summary`, `input_tokens`, `output_tokens`, `processing_ms`, `retry_count` (SMALLINT NOT NULL DEFAULT 0) |
...
3. No existing table is altered — M3 adds new nullable columns only.
4. No existing column is renamed or dropped.
5. The migration uses `CREATE TABLE IF NOT EXISTS` for new tables and `ALTER TABLE ... ADD COLUMN IF NOT EXISTS` for new columns.
6. The migration must be tested against a copy of the existing production schema before merging.
7. Rollback of application code must not corrupt existing data. New columns have safe defaults (`retry_count DEFAULT 0`); all other new columns are nullable.
8. If the migration runner's DB user lacks privileges to alter superuser-owned tables, use guarded DO blocks or safe side-table patterns.
